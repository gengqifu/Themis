...
- [ ] AC3: 支持路径豁免（glob，匹配**仓库相对路径**，默认区分大小写）
...
- [ ] AC5.2: baseline 指纹为 `rule_id + file + line + normalized_match_hash`（normalized_match_hash = 去空白后的匹配文本 SHA256）
...
- [ ] 6. **实现**：diff 输入解析与数据结构
- [ ] 7. **实现**：`diff` 模式扫描（只读指定行）
- [ ] 8. **实现**：`full` 模式开关
- [ ] 9. **实现**：路径豁免与注释豁免
- [ ] 10. **实现**：baseline schema 与过滤逻辑（仅指纹）
- [ ] 11. **重构**：简化输入管道与豁免判断
