...
- [ ] **测试**：配置加载（默认路径/参数覆盖/错误提示/未知字段报错）
- [ ] **测试**：规则匹配与行号定位（命中位置准确）
- [ ] **测试**：输出格式（text/json；脱敏默认前后 2 字符）
- [ ] **测试**：退出码（0/1/2 对应成功/错误/阻断）
- [ ] **测试**：二进制/大文件跳过逻辑（默认阈值与判定规则）
- [ ] **实现**：建立 Python 包骨架与 CLI 入口（最小可运行）
- [ ] **实现**：CLI 入口提供 `themis` 可执行脚本，并兼容 `python -m themis`
- [ ] **实现**：配置加载与 schema 校验
- [ ] **实现**：配置 schema 明确字段与类型（`scan/rules/output` 等）
- [ ] **实现**：扫描器（逐文件/逐行扫描 + 结果结构）
- [ ] **实现**：输出格式化与退出码约定
- [ ] **实现**：默认规则集归档到独立函数/模块，便于测试断言
- [ ] **重构**：清理重复逻辑，简化接口
